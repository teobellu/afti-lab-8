...
public class Entry {
	...
	public List<Box> computeBoxes() {
		 ...
		 * @since 1.5
		 */
		if (AppInputRuntimeInterfacer.only_u)
			return computeBoxesNoRest_ONLY_U();
	 ...
	 * @return
	 */
	private List<Box> computeBoxesNoRest_ONLY_U() {
		List<Box> boxes = new ArrayList<Box>();
		...
		for (String size : rqtmap.keySet()) {
			...
			if (qsize > 0) {
				Box box = new Box(title);
				box.intern.put(size, qsize); //XL 13
				box.scale = 1; //1
				boxes.add(box);
				// equals to rqtmap.clear(...)
				rqtmap.put(size, qsize - (box.scale * qsize));
			}
		}
